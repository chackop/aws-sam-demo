...
```bash
aws s3 mb s3://s3bucketname-region global
sam package --template-file template.yaml --output-template-file packaged.yml --s3-bucket chx-test-sam-project
sam deploy --capabilities CAPABILITY_IAM --template-file packaged.yml --stack-name test-sam-project
sam logs --name ClockFunction --stack-name test-sam-project --region us-east-1 
```
